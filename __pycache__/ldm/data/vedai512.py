...
class VEDAIBase(Dataset):
    def __init__(self,
        ...
        if txt_file is not None:
            ...
            # co -> ir naming convention for VEDAI
            co_files = sorted([
                f for f in os.listdir(self.vi_data_root)
                if f.endswith(('jpg', 'png'))
            ])
            # Map visible co files to corresponding IR filenames
            self.image_paths = [
                f.replace('co', 'ir') for f in co_files
            ]
