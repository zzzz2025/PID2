...
class KAISTBase(Dataset):
    def __init__(self,
        ...
        # Auto-discover files by scanning the IR directory
        ir_files = sorted([
            f for f in os.listdir(self.ir_data_root)
            if f.endswith(('jpg', 'png'))
        ])
        # Use txt_file only if explicitly provided
        if txt_file is not None:
            with open(txt_file, "r") as f:
                lines = f.read().splitlines()
            # txt contains relative paths -> map to IR filenames
            ir_names = {os.path.basename(l): l for l in lines}
            self.image_paths = [ir_names[f] for f in ir_files if f in ir_names]
        else:
            self.image_paths = ir_files
...
class KAISTTrain(KAISTBase):
    ...
    def __getitem__(self, i):
        ...
        example["rgb_image"] = example.pop("conditional")
        return example
